...
function callTracking3(ssotoken) {
    //window.open("callTracking2.html?apikey="+apiKey+"&token="+ssotoken+"&server=" + server + "", "_blank")

    console.log('ssoToken'+' '+ ssotoken );
    console.log('apiKey'+' '+ apiKey);
    console.log('server'+' '+ server)
    var xhr = new XMLHttpRequest();
    var url = "https://"+server+".agendize.com/api/2.0/calls/calltrackings?apikey=" + apiKey + "&token=" + ssotoken + "&server=" + server ;
        ...
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            for (i = 0; i < json.items.length; i++) {
                //  
                    ...
                    odd = !odd;
                    document.write(">");
                    document.write("<td>" + json.items[i].owner.userName + "</td>");
                    ...
                    document.write("<td>" + json.items[i].phoneNumber + "</td>");
